...
- **Traffic Management / Status Message shown "Unsupported" on capable firmware (#3457)**: Device Configuration inferred module support from the presence of a decoded module-config sub-message, but Proto3 omits a sub-message whose every field is default — so a node that had never configured a supported module looked unsupported. Version 4.10.3 switched to firmware-version gates. **Compatibility correction:** later released-source verification established that Status Message requires 2.7.20+, while Traffic Management is absent from released 2.7.x firmware and requires 2.8.0+; current releases use those corrected thresholds.
...
- **Traffic Management telemetry surfaced in graphs**: MeshMonitor ingests and stores the module's `TrafficManagementStats` counters (packets inspected, position-dedup drops, NodeInfo cache hits, rate-limit drops, unknown-packet drops, hop-exhausted packets, router hops preserved). Those seven counters now render as labelled, integer-valued series in a node's telemetry graphs under a shared **"Traffic Mgmt:"** label group (previously they were stored but shown with raw field names). **Compatibility correction:** released 2.7.x firmware does not contain Traffic Management; the module requires firmware 2.8.0+.
